...
LUNA_SUBSET_ROOT_DIR = 'data-unversioned/luna'
LUNA_ANNOTATIONS_ROOT_DIR = 'data/luna'
...
raw_cache = getCache('dsets_cls_raw')
...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    # the subsets yet.
    mhd_list = glob.glob(f'{LUNA_SUBSET_ROOT_DIR}/subset*/*.mhd')
    presentOnDisk_set = {os.path.split(p)[-1][:-4] for p in mhd_list}

    candidateInfo_list = []
    with open(f'{LUNA_ANNOTATIONS_ROOT_DIR}/annotations_with_malignancy.csv', "r") as f:
    ...
    with open(f'{LUNA_ANNOTATIONS_ROOT_DIR}/candidates.csv', "r") as f:
        for row in list(csv.reader(f))[1:]:
            series_uid = row[0]
...
class Ct:
    def __init__(self, series_uid):
        mhd_path = glob.glob(
            '{}/subset*/{}.mhd'.format(LUNA_SUBSET_ROOT_DIR, series_uid)
        )[0]
